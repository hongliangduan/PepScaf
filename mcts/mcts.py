...
log.info(
    f'Number of data with enrich > {enrich_cut} and len = {pep_len} is {n}')
...
def second(scaffold: Scaffold):
    ...
    pep_list = [''.join(scaffold.span(action_set=action_set))
                for i in range(n)]
    pep_set = set(pep_list)
    log.info(f'data num is {len(pep_set)}')
    score = 0.
    with Progress(SpinnerColumn(), *Progress.get_default_columns(), "Elapsed:", TimeElapsedColumn(), transient=False) as progress:
