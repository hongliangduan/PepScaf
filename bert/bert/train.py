...
class Trainer(object):
    ...
    def __init__(self, cfg, model, data_iter, optimizer, save_dir, device, log_dir=None):
        ...
        for e in range(self.cfg.n_epochs):
            ...
            with Progress(SpinnerColumn(), *Progress.get_default_columns(), "Elapsed:",
                ...
                try:
                    ...
                    _len = None
                for i, batch in progress.track(enumerate(self.data_iter), description='Iter', total=_len):
                    ...
                    if self.cfg.total_steps and self.cfg.total_steps < global_step:
                        log.info('Epoch %d/%d : Average Loss %5.3f' %
                        ...
                        return
            log.info(
                'Epoch %d/%d Step %d: Average Loss %6.4f' % (e + 1, self.cfg.n_epochs, global_step, loss_sum / (i + 1)))
            if self.log_dir:
                writer.add_scalars(
                    'loss', {'loss': loss_sum / (i + 1)}, global_step)
        self.save(global_step)
        ...
        with Progress(SpinnerColumn(), *Progress.get_default_columns(), "Elapsed:", TimeElapsedColumn()) as progress:
            for batch in progress.track(self.data_iter, description='Iter'):
                ...
                with torch.no_grad():  # evaluation without gradient calculation
                    accuracy, result, logit, label_id, batch_attns, pooled_h = evaluate(model,
                                                                                        batch)  # accuracy to print
