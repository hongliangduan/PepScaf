...
pep_gens_df = pd.DataFrame({'enrichment': [0] * n_negatives, 'seq': pep_gens, 'label': [0] * n_negatives})
df['label'] = 1
...
balanced_df = pd.DataFrame(
    {'seq': df_all.seq, 'label': df_all.label})

train_df, test_df = split_df(balanced_df, shuf=True, val=False, random_state=seed)  # 8:2
