...
SeqIO.write(record_iters(df["seq"], df.index, df["enrichment"]), FASTA, "fasta")
...
neg_peps_df = pd.DataFrame(
    {"seq": neg_peps, "enrichment": [0] * n_negs, "label": [0] * n_negs}
)
...
df_balanced_target_cluster = pd.concat([pos_peps_df, neg_peps_df]).reset_index(
    drop=True
)
# df_balanced_target_cluster.to_csv(BALCANCED_CSV)
pd.DataFrame(
    {
        "seq": df_balanced_target_cluster["seq"],
        "label": df_balanced_target_cluster["label"],
    }
).to_csv(
    BALCANCED_CSV
)  # for non-pretrain
